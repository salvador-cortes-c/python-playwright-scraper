...
def _replace_last_nth_child(selector: str, nth: int) -> str:
    """Replace the last `nth-child(N)` in a selector with the provided N.

    This helps when users paste full-page selectors generated by devtools that
    include a specific product-card index (e.g. nth-child(19)).
    """
...
async def _text_for_card_selector(page, card, selector: str, nth_in_parent: int) -> str:
    if not selector:
        return ""
    ...
    # Fallback for absolute selectors that include a specific nth-child() index.
    if "nth-child" in selector:
        try:
            adjusted = _replace_last_nth_child(selector, nth_in_parent)
            loc = page.locator(adjusted).first
            if await loc.count() > 0:
                return (await loc.inner_text()).strip()
        except Exception:
            return ""
...
async def scrape_products(
    ...
    for index in range(count):
        ...
        nth_in_parent = 1
        if "nth-child" in promo_price_dollars_selector or "nth-child" in promo_price_cents_selector or "nth-child" in promo_unit_price_selector:
            try:
                nth_in_parent = int(
                    await card.evaluate(
                        "el => el && el.parentElement ? Array.from(el.parentElement.children).indexOf(el) + 1 : 1"
                    )
                )
            except Exception:
                nth_in_parent = 1
        ...
        promo_price_dollars = await _text_for_card_selector(
            page=page,
            card=card,
            selector=promo_price_dollars_selector,
            nth_in_parent=nth_in_parent,
        )
        promo_price_cents = await _text_for_card_selector(
            page=page,
            card=card,
            selector=promo_price_cents_selector,
            nth_in_parent=nth_in_parent,
        )
        promo_unit_price = await _text_for_card_selector(
            page=page,
            card=card,
        ...
        promo_price = ""
        promo_cleaned_cents = (promo_price_cents or "").replace(".", "").strip()
        promo_dollars = (promo_price_dollars or "").strip()
        if promo_dollars and promo_cleaned_cents:
            promo_price = f"{promo_dollars}.{promo_cleaned_cents}"
        elif promo_dollars:
            promo_price = promo_dollars
